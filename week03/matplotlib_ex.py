...
pclass_survivied_mean = titanic.groupby('Pclass')['Survived'].mean().reset_index()
pclass_survivied_mean
...
plt.close()                         # 다음 Plot를 새로 그리기 위해 plt 닫기
...
# 두 번째 서브프롯 설정(막대 그래프)
plt.subplot(2, 1, 2)
plt.bar(x, y2, color='deeppink', alpha=0.7, width=9.5, label='Survived')
plt.xlabel('Parch')
...
# 각 부모와 자녀의 수에 따른 생존자와 사망자 수 계산
parch_counts = titanic.groupby('Parch')['Sruvived'].value_counts().unstack().fillna(0)
print(parch_counts)

# x, y값 설정
x = parch_counts.index.astype(str) # 데이터프레임의 인덱스를 문자열로 변환
y1 = parch_counts[0].value # 사망자
y2 = parch_counts[1].value # 생존자
